...
- **Automated tooling**: a hook, the same mechanism described earlier, firing a script such as a linter, a test suite, a formatter, or a quality check, checking the output against rules that need no judgment at all.
...
<ThemedIllustration
  ...
  alt="A flow chart of five workflow phases in sequence: brainstorming, planning, execution, review, and documentation. Each step lists its model, the context it reads or writes, and its gate type, human, automated tooling, or agent verdict. A branch from review loops back to planning for rework, or continues on to documentation once approved."
